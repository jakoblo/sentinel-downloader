...
def download(file, bbox, buffer=0, scale_factor=0, grid=0):
  ...
  with rasterio.open(file) as src:
      ...
      if grid != 0:
        ...
        if w.width % 128 != 0:
            window_width = w.width+(128-w.width % 128)
        if w.height % 128 != 0:
            window_height = w.height+(128-w.height % 128)
        w = win.Window(col_off=w.col_off, row_off=w.row_off, width=window_width, height=window_height)
...
if __name__ == '__main__':
  ...
  for f in files:
    ...
    if b == "B11" or b == "B12":
      scale = 2
    img, meta, tf = download(f, bbox, buffer=50, scale_factor=scale, grid=0)
